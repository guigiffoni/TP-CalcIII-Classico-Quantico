...
A distribuição espacial desse campo térmico pode ser visualizada na Figura 1, que ilustra a intensidade do \textit{hotspot} sobre a área do chip.
...
Como evidenciado na Figura 1, uma simples medição pontual seria insuficiente para determinar o estado térmico global do componente. Por isso, aplicamos a integral dupla sobre toda a região do domínio para calcular o volume sob esta superfície de temperatura. Dividindo esse volume pela área total, obtemos a temperatura média real dissipada pelo chip ($\bar{T} = 66,28^{\circ}C$), que servirá como base para a nossa modelagem quântica subsequente.
...
\subsubsection{Verificação com WolframAlpha\cite{wolframalpha} e SciPy\cite{scipy}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{fase2/img/esfera-bloch3.png}
